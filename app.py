...
st.markdown("Ingresá tus activos para proyectar a 10 años y ver su rendimiento histórico.")
...
with st.sidebar.form("input_form"):
    ...
    if add_btn and ticker:
        if len(st.session_state.lista_instrumentos) < 50:
            ...
            st.error("Límite de 50 alcanzado")
...
if st.session_state.lista_instrumentos:
    ...
    for _, row in df_input.iterrows():
        ...
        try:
            ...
            if not data.empty:
                ...
                riesgo = "Bajo 🟢" if vol < 0.18 else "Medio 🟡" if vol < 0.40 else "Alto 🔴"
        ...
        resultados.append({
            ...
            "Riesgo": riesgo,
            "Valor en 10 años ($1000)": round(proyeccion, 2)
        })
    ...
    with col1:
        ...
        st.download_button("📥 Descargar Excel (CSV)", csv, "mi_cartera.csv", "text/csv")

    with col2:
        st.subheader("📈 Rendimiento Últimos 12 Meses")
        if tickers_grafico:
